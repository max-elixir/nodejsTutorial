...
router.post('/', [auth, [check('text', 'Text is required').not().isEmpty()]],
    async(req, res) => {
        ...
        try {
            ...
            res.status(500).send('Server Error: create post');
        }
});
...
router.delete('/:id', auth, async (req, res) => {
    try {
        ...
        // Check user is owner of deleted post
...
router.put('/unlike/:id', auth, async (req, res) => {
    try {
        ...
        if(post.likes.filter(like=> like.user.toString() === req.user.id).length === 0){
            return res.status(400).json({ msg: 'Post wasn\'t liked yet'});
        }
...
router.post('/comment/:id', [auth, [check('text', 'Text is required').not().isEmpty()]],
    async(req, res) => {
        ...
        try {
            ...
            console.error(err.message);
            res.status(500).send('Server Error: create comment');
        }
});
...
router.delete('/comment/:id/:comment_id', auth, async (req, res) =>{
    try {
        ...
        // check if user owns comment to be deleted
...
//todo -- include an update comment, might have to teach myself
